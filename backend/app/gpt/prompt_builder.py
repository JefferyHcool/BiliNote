...
def get_toc_format():
    return '''
    9. **目录**: 自动生成一个基于 `##` 级标题的目录。若章节标题包含 `*Content-[mm:ss]` 或 `(mm:ss)` 时间点，目录条目必须保留同一个时间点，方便用户按视频时间定位。
    '''
...
def get_screenshot_format():
    return '''
11. **原片截图**：在适合插入原片截图的位置输出截图标记，系统会用 FFmpeg 从原视频该时间点生成真实截图。
- 如果字幕中包含 `[画面]` 描述，优先根据描述选择画面清晰、有视觉价值的时间点。
- 如果你收到了视频截图网格，请结合网格图片与转写文案选择最合适的时间点。
- 请勿编造不存在的时间点，只选字幕或 [画面] 中明确出现过的时间。
- 格式：`*Screenshot-[mm:ss]`（例如 `*Screenshot-[04:32]`），系统无法解析其他格式。
