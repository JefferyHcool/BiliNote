...
class TestMessageTokenEstimate(unittest.TestCase):
    """覆盖图片 payload 的预算估算，避免 base64 数据被重复计入。"""

    def test_text_estimate_uses_cjk_aware_heuristic(self):
        gpt = _make_gpt()

        self.assertEqual(gpt._estimate_text_tokens("中文测试"), 4)
        self.assertEqual(gpt._estimate_text_tokens("abcdefgh"), 2)

    def test_base64_image_uses_fixed_image_budget(self):
        gpt = _make_gpt()
        data_url = "data:image/jpeg;base64," + ("A" * 10000)
        messages = gpt.create_messages(segments=[], video_img_urls=[data_url])

        scrubbed = gpt._scrub_image_payloads_for_token_estimate(messages)
        text_tokens = gpt._estimate_text_tokens(json.dumps(scrubbed, ensure_ascii=False))

        self.assertEqual(
            gpt._estimate_messages_tokens(messages),
            text_tokens + gpt.image_token_estimate + 256,
        )

    def test_remote_image_url_uses_fixed_image_budget(self):
        gpt = _make_gpt()
        messages = gpt.create_messages(
            segments=[],
